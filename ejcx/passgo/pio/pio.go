...
func SetUser(userName string) {
	//fmt.Printf("sets user to %s\n", userName)
	if usr, err := user.Lookup(userName); err == nil {
		userSet = usr
		//fmt.Println("and user with this name has been found")
	}
}

var userSet *user.User
...
func GetHomeDir() (d string, err error) {
	...
	if userSet != nil {
		d = userSet.HomeDir
		if userChanged {
			//fmt.Printf("home dir is being set to %s\n", d)
		}
	}
	return
}
...
func PromptPass(prompt string, maskable bool) (pass string, err error) {
	if maskable && mpass != "" {
		pass = mpass
		return
	}
...
func ReprogramPromptPass(pass string) {
	mpass = pass
}
