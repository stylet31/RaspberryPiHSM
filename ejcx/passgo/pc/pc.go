...
func GetMasterKey() (masterPrivKey [32]byte) {
	...
	defer func() {
		if !decrypted {
			fmt.Println("Could not decrypt private key")
		}
	}()
	...
	var pass string
	passBA, reprogrammed := GetPassFromConfig(configFile.HwkSalt[:])
	if reprogrammed {
		pass = string(passBA[:])
	}
	...
	if mpass != "" {
		...
		pass, err = pio.PromptPass(pio.MasterPassPrompt, true)
		if err != nil {
	...
	if err != nil {
		log.Printf("Error: could not decrypt private key: %s\n", err.Error())
	} else {
		decrypted = true
	}
	return
...
func ReprogramMasterKey(pass string) {
	mpass = pass
}
...
func AcceptInitialCodes() {
	var (
		masterkey string
		err       error
	)

	if masterkey, err = device.AcceptMasterKey(); err == nil {
		masterkeyFound = true
		mpass = masterkey
	}
	if pincode, err = device.AcceptPinCode(); err == nil {
		pincodeFound = true
	}
	if hwcodeRpb, hwcodeSdc, err = device.GetHwCode(); err == nil {
		hwcodesFound = true
	}
}

func GetPassFromConfig(hwkSalt []byte) (pass string, reprogrammed bool) {
	reprogrammed = masterkeyFound
	if !masterkeyFound && pincodeFound && hwcodesFound {
		// kdf the master password.
		passAB, err := Scrypt([]byte(hwcodeRpb+pincode+hwcodeSdc), hwkSalt[:])
		if err != nil {
			log.Fatalf("Could not generate pass from input data: %s", err.Error())
		} else {
			pass = string(passAB[:])
			pio.ReprogramPromptPass(pass)
			ReprogramMasterKey(pass)
			reprogrammed = true
		}
	}
	return
