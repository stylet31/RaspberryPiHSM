...
// obtainHwCodes will initialize a new password vault in the home directory.
func Init() {
	...
	if needsDir {
		...
		if err != nil {
			log.Fatalf("Could not create passgo qvault: %s", err.Error())
		}
	}

	// Don't just go around deleting things for users or prompting them
	// to delete things. Make them do this manually. Maybe this saves 1
	// person an afternoon.
	if hasConfig {
	...
	// Create a new salt for encrypting hardware code.
	var hwkSalt [32]byte
	_, err = rand.Read(hwkSalt[:])
	if err != nil {
		log.Fatalf("Could not generate random salt: %s", err.Error())
	}

	pass, reprogrammed := pc.GetPassFromConfig(hwkSalt[:])
	if !reprogrammed {
		pass, err = pio.PromptPass("Please enter a strong master password", true)
		if err != nil {
